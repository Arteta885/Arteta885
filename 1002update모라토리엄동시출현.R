...
ggraph(graph_pair, layout = "fr") +
  geom_edge_link(aes(edge_alpha = n), show.legend = FALSE) +  # 엣지 투명도 조정
  scale_edge_alpha(range = c(0.3, 1)) +  # 엣지 투명도 범위 설정 (0.3 ~ 1)
  geom_node_point(aes(size = n, color = n)) +  # 크기는 적용하되 범례에서 제거하지 않음
  scale_size(range = c(3, 15), guide = "none") +  # 크기 범례는 제거
  scale_color_viridis_c() +  # 색상 스케일 설정
  geom_node_text(aes(label = name), repel = TRUE, size = 3) +  # 노드 텍스트
  labs(title = "Pairwise Co-occurrence Network 모라토리엄(n>=80)", color = "Frequency") +  # 색상 범례만 표시
  theme_void()  # 테마 설정
